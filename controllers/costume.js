...
// Handle a show-one view with id specified by query string, e.g. /costumes/detail?id=...
exports.costume_view_one_Page = async function(req, res) {
    console.log("single view for id " + req.query.id)
    try {
        let id = req.query.id;
        if (!id) {
            res.status(400).send('{"error": "Missing id query parameter"}');
            return;
        }
...
// Handle the all-costumes page (list view)
